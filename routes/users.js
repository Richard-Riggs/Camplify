...
// NEW USER ROUTE
router.get("/users/new", middleware.isLoggedOut, (req, res) => {
...
router.post(
  "/users",
  [middleware.isLoggedOut, upload.single("image")],
  async (req, res, next) => {
    try {
      let image, imageID;
      if (req.file) {
...
router.get("/users/:username", (req, res, next) => {
  ...
  User.findOne({ username: req.params.username })
    .populate("campFavs.campID")
    .exec(function(error, user) {
      if (error) return next(error);
      else if (!user)
        return next({ name: "Error", message: "User does not exist" });
      else {
        ...
        if (!update) {
          ...
          switch (userTab) {
            case "list-recent":
              Campground.find({ "author.id": user._id }, function(
                error,
                campgrounds
              ) {
                if (error) return next(error);
                else {
                  Review.find({ "author.id": user._id })
                    .populate("campground")
                    .exec(function(error, reviews) {
                      if (error) return next(error);
                      else {
                        let recentActivities = user.campFavs
                          .concat(campgrounds)
                          .concat(reviews);
                        ...
                        return res.render("partials/user-recent", {
                          ...
                          recentActivities: recentActivities.slice(
                            firstItem,
                            lastItem
                          )
                        });
            ...
            case "list-campgrounds":
              Campground.find({ "author.id": user._id })
