      ...
      Campground.create(
        {
          ...
          location: `${data[0].city ? (data[0].city + ", ") : ""}${data[0].administrativeLevels.level1short}, ${data[0].country}`,
          lat: data[0].latitude,
...
router
    ...
    function(req, res, next) {
      geocoder.geocode(req.body.location, function(error, data) {
        ...
        Campground.findById(req.params.id, async function(error, campground) {
          ...
          campground.location = `${data[0].city ? (data[0].city + ", ") : ""}${data[0].administrativeLevels.level1short}, ${data[0].country}`;
  ...
  .delete(
    [middleware.isLoggedIn, middleware.checkCampgroundOwnership],
    function(req, res, next) {
      Campground.findById(req.params.id, async function(error, campground) {
        if (error) return next(error);
...
router
  ...
  // EDIT ROUTE
  .get([middleware.isLoggedIn, middleware.checkCampgroundOwnership], function(
    req,
    res
  ) {
