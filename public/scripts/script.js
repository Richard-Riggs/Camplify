...
$(function() {
  ...
  if ($('#user-tabs-list .active').length) {
    ...
    $('#user-tabs-list a').on('click', function(event) {
      (currentTab = $(this)[0].id), (currentContent = $(this)[0].hash), (currentPage = 1);
      $.get(URLpath, {
  ...
  const allowSubmit = function(validInputs) {
    if (validInputs.profilePicture && validInputs.profileVisibility && validInputs.email && validInputs.password)
      return true;
    else return false;
  };
  ...
  // Ensures that input fields are readonly when they are hidden from user
  $('#settings-content').on('show.bs.collapse', '.collapse', function(event) {
    $(this).find('input').prop('readonly', false);
  });
  $('#settings-content').on('hidden.bs.collapse', '.collapse', function(event) {
  ...
  $('#settings-content').on('change', 'input[type=password]', function(event) {
    ...
    if (allValued) {
      settingStatus.html('Change password now');
    } else {
      settingStatus.html(settingStatus.attr('data-default'));
    }

    if ($('#current-password').val() && $('#current-password').val() === $('#new-password').val()) {
  ...
  $('#settings-content').on('change', 'input[type=email]', function(event) {
    ...
    if (allValued) {
      settingStatus.html(emails.get(0).value);
    } else {
      settingStatus.html(settingStatus.attr('data-default'));
    }
  ...
  $('#settings-content').on('submit', '#settings-form', function(event) {
    ...
    $.post(actionURL, $(this).serialize(), function(data) {
      if (data.errorField && data.errorField === 'password') {
        ...
        $('#password-settings-header .setting-status').removeClass('text-success').addClass('text-danger');
        validInputs.password = false;
        $('#settings-submit').prop('disabled', !allowSubmit(validInputs));
      } else {
        $('#settings-content').html($.parseHTML(data));
        $('#settings-success').fadeTo(400, 1);
      }
    });
  });

  // Removes 'Settings Saved' notice when the user inputs new data
  $('#settings-content').on('change', 'input', function(event) {
    $('#settings-success').fadeTo(400, 0);
  });
});
